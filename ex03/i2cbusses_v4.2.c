...
static enum adt i2c_get_funcs(int i2cbus)
{
	...
	__fi_i2c = open_i2c_dev(i2cbus, filename, sizeof(filename), 1);
	if (__fi_i2c < 0)
		return adt_unknown;

	if (ioctl(__fi_i2c, I2C_FUNCS, &funcs) < 0)
		ret = adt_unknown;
...
struct i2c_adap *gather_i2c_busses(void)
{
	...
	if ((__ffh_i2c = fopen("/proc/bus/i2c", "r"))) {
		while (fgets(s, 120, __ffh_i2c)) {
	...
	if ((__ffh_i2c = fopen("/proc/mounts", "r")) == NULL) {
		goto done;
	}
	while (fgets(n, NAME_MAX, __ffh_i2c)) {
		sscanf(n, "%*[^ ] %[^ ] %[^ ] %*s\n", sysfs, fstype);
...
int open_i2c_dev(int i2cbus, char *filename, size_t size, int quiet)
{
	...
	__fi_i2c = open(filename, O_RDWR);

	if (__fi_i2c < 0 && (errno == ENOENT || errno == ENOTDIR)) {
		len = snprintf(filename, size, "/dev/i2c-%d", i2cbus);
		if (len >= (int)size) {
			fprintf(stderr, "%s: path truncated\n", filename);
			return -EOVERFLOW;
	...
	if (__fi_i2c < 0 && !quiet) {
		if (errno == ENOENT) {
			fprintf(stderr, "Error: Could not open __fi_i2c "
...
int set_slave_addr(int ___fh_i2c, int address, int force)
{
	/* With force, let the user read from/write to the registers
	   even when a driver is also running */
	if (ioctl(___fh_i2c, force ? I2C_SLAVE_FORCE : I2C_SLAVE, address) < 0) {
		fprintf(stderr,
